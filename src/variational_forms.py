...
def define_variational_forms(epsilon, sigma_func, H, psi_func, pold, u, v, p, q, unew, Hold, data, boundary_markers, E_expr, nu):
    Gc = data["Gc"]
    ...
    E_phi = (Gc * l * inner(grad(p), grad(q)) \
             + ((Gc / l) + 2.0 * H(Hold, data, psi)) * inner(p, q) \
             - 2.0 * H(Hold, data, psi) * q) * dx
